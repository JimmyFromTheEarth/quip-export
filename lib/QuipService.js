...
class QuipService {
    ...
    async download(url) {
        ...
        try{
            ...
            if(this._checkRetryQuery(url)){
                this.logger.error(`Couldn't download [${url}], tryed to get it ${TIMES_LIMIT_429} times`);
    ...
    async _executeApiCall(url, method, blob, options = {}) {
        ...
        try {
            ...
            this.logger.error(`Couldn't fetch ${this.apiURL}${url}, `, e);
        }
    }
    ...
    async _handleApiCallResponseError(res, url, method, blob, options = {}) {
        ...
        if(res.status === 503 || res.status === 429) {
            ...
            this.logger.debug(`Couldn't fetch ${this.apiURL}${url}, received ${res.status}, ${await res.text()}`);
            return;
        }
    }
