...
class QuipService {
    ...
    async _apiCall(url, method, blob) {
        ...
        if(this.rateLimitRemaining <= 0){
            //console.log(`Delayed`);
            return await this._apiCallInDelay(url, method, blob, this.waitingMs); 
        }
        ...
        if(this.delayCalls === true){
            ...
            //console.log(`\r\nImediate Call`);
            return await this._executeApiCall(url, method, blob);
        }
    }
    ...
    async _apiCallInDelay(url, method, blob, waitingMs) {
        await this._delay(waitingMs);
        return await this._executeApiCall(url, method, blob);
    }

    async _executeApiCall(url, method, blob) {
        //console.log(`\r\nExecute API Call`);
        try {
            const res = await fetch(`${this.apiURL}${url}`, this._getOptions(method));

            return await this._handleApiCallResponse(res, url, method, blob);
            
        } catch (e) {
            //console.log(`Couldn't fetch: ${url} ==> ${e}`);   
            this.logger.error(`Couldn't fetch ${url}, `, e);
        }
    }

    async _handleApiCallResponseError(res, url, method, blob) {
        //
        if(res.status === 503 || res.status === 429) {
            ...
            if(this._checkRetryQuery(url)) {
                return await this._apiCallInDelay(url, method, blob, waitingInMs);
            } else {
                //console.log(`\r\nCouldn't fetch ${url}, tryed to get it ${TIMES_LIMIT_429} times`);
    ...
    async _handleApiCallResponse(res, url, method, blob) {
        //
        ...
        //console.log(`Rate Limit: Limit: [${headers.rateLimitLimit}] - Reset: [${headers.rateLimitReset}} MS ] - Remaining: [${headers.rateLimitRemaining}]`);

        if(!res.ok) {
            return await this._handleApiCallResponseError(res, url, method, blob);
        }
