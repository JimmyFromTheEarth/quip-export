...
class QuipProcessor {
    ...
    async startExport(folderIds) {
        ...
        this.indexFileWriteStream.write(`"Path","Link","Last Modified Date/Time"\n`);
        //}
    ...
    async _writeIndexLine(path, quipThread) {
        ...
        this.indexFileWriteStream.write(`"${path}${quipThread.thread.title}","${quipThread.thread.link}","${updateTime.toISOString()}"\n`); // path already has a /
    }
