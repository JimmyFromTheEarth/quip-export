...
class QuipProcessor {
    constructor (quipToken, saveCallback = ()=>{}, progressCallback = ()=>{}, phaseCallback = ()=>{}, options={}) {
        ...
        this.excludeFoldersMatchingRegex = null

        if (this.options.excludePattern) {
            this.excludeFoldersMatchingRegex = new RegExp(this.options.excludePattern);
        }
        ...
        this.indexFileWriteStream = null
    }
    ...
    setWaitingMs(minuteRateLimits) {
        this.quipService.setWaitingMs(minuteRateLimits);
    }

    setForceDelay() {
        this,this.quipService.setForceDelay();
    }

    async startExport(folderIds) {
        console.log(`Start to export ...`);
        //if (this.options.onlyIndex) {
        this.indexFileWriteStream = await fs.createWriteStream(`${this.options.destinationFolder}/${this.options.indexFileName ? this.options.indexFileName : 'index'}.csv`);
        this.indexFileWriteStream.write(`"Thread ID",Folder,Titile,Type,"Quip Path",Link,"Last Modified Date/Time"\n`);
        //}
        ...
        console.log(`USER-URL: ${this.quipUser.url}`);
    ...
    stopExport() {
        ...
        //if(this.options.onlyIndex && this.indexFileWriteStream){ this.indexFileWriteStream.end(); }
        if(this.indexFileWriteStream){ this.indexFileWriteStream.end(); }
    }
    ...
    async _processDocumentThreadPDFRequest(quipThread) {
        await this.quipService.createExportPdfRequest(quipThread.thread.id);
    }

    async _processDocumentThreadPDF(quipThread, path) {
        const pdf = await this.quipService.exportToPDF(quipThread.thread.id);
        if(pdf) {
            this.saveCallback(pdf, sanitizeFilename(`${quipThread.thread.title.trim()}.pdf`), 'BLOB', path);
        }
    }

    async _processSlidesThreadPDF(quipThread, path) {
        const pdf = await this.quipService.getPdf(quipThread.thread.id);
        if(pdf) {
            this.saveCallback(pdf, sanitizeFilename(`${quipThread.thread.title.trim()}.pdf`), 'BLOB', path);
        }
    }
    ...
    async _writeIndexLine(path, quipThread) {
        const updatedTimeMicrosecondsSinceEpoc = quipThread.thread.updated_usec;
        const updateTime = new Date(updatedTimeMicrosecondsSinceEpoc / 1000);

        this.indexFileWriteStream.write(`${quipThread.thread.id},"${path}","${quipThread.thread.title}","${quipThread.thread.type}","${path}${quipThread.thread.title}","${quipThread.thread.link}","${updateTime.toISOString()}"\n`); // path already has a /
    }
    ...
    async _processThread(quipThread, path) {
        ...
        */
        ...
        await this._writeIndexLine(path, quipThread);

        if (this.options.onlyIndex) { return 0; } 
        //else {

        if(this.options.docxArchiveInit) {
            if(quipThread.thread.type !== 'spreadsheet') {
                await this._processDocumentThreadPDFRequest(quipThread);
            }
        } else if(this.options.docxArchive){
            if(quipThread.thread.type === 'spreadsheet') {
                await this._processDocumentThreadXlsx(quipThread, path);
            } else {
                await this._processDocumentThreadPDF(quipThread, path);
            }
        }
        else if(this.options.docx) {
            ...
            if(quipThread.thread.type === 'slides') {
                await this._processSlidesThreadPDF(quipThread, path);
            }else{
    ...
    async _processThreads(quipThreads, path) {
        ...
        for(const index in quipThreads) {
            ...
            await this._processThread(quipThreads[index], path);
        }
    ...
    async _processFolders(quipFolders, path) {
        ...
        for(const index in quipFolders) {
            const currentFolderTitle = sanitizeFilename(quipFolders[index].folder.title);
            let allowThisToPass = true;
            if ( this.excludeFoldersMatchingRegex && this.excludeFoldersMatchingRegex.test(currentFolderTitle.toLowerCase()) ) {
                allowThisToPass = false;
            }

            if (allowThisToPass) {
                //promises.push(this._processFolder(quipFolders[index], `${path}${currentFolderTitle}/`));
                await this._processFolder(quipFolders[index], `${path}${currentFolderTitle}/`);
            }
        }
        //await Promise.all(promises);
    }
    ...
    async _processFolder(quipFolder, path) {
        ...
        this._progressReport({
            ...
            nextCallInSeconds: utils.normalizeInteger(this.quipService.waitingMs) / 1000,
            rateLimitsResetAfter: (utils.normalizeInteger(this.quipService.rateLimitResetMs) - Date.now()) / 1000,
            limitsRmaining: this.quipService.rateLimitRemaining,
            failedCallsTotal: this.quipService.queriesRetry.size,
            path: path
        });
    }
