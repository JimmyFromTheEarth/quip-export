...
function normalizeInteger (source) {
    let parsed;

    //console.log(`normalizeInteger ==> ${source}, is integer ${Number.isInteger(source)}, is finite ${Number.isFinite(source)}, parse float ${Number.parseFloat(source)}`);

    try {
        parsed = Number.parseInt(source);
    }catch(ex) {
        return parsed;
    }
